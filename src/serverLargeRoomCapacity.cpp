...
LargeRoomCapacity::LargeRoomCapacity(std::string capacity) :
		RoomCapacity(std::move(capacity), LARGE_ROWS_QUANTITY,
					 LARGE_COLUMNS_QUANTITY) {
...
LargeRoomCapacity::LargeRoomCapacity(const LargeRoomCapacity &other)
		: RoomCapacity(
		other.capacity_name,
		other.rows_quantity,
		other.columns_quantity) {}

LargeRoomCapacity::LargeRoomCapacity(LargeRoomCapacity &&other) noexcept
		: RoomCapacity(
...
std::string LargeRoomCapacity::getSeats() const {
...
void LargeRoomCapacity::book(char row, int column) {
