...
SmallRoomCapacity::SmallRoomCapacity(std::string capacity) :
		RoomCapacity(std::move(capacity), SMALL_ROWS_QUANTITY,
					 SMALL_COLUMNS_QUANTITY) {
...
SmallRoomCapacity::SmallRoomCapacity(const SmallRoomCapacity &other)
		: RoomCapacity(
		other.capacity_name,
		other.rows_quantity,
		other.columns_quantity) {}

SmallRoomCapacity::SmallRoomCapacity(SmallRoomCapacity &&other) noexcept
		: RoomCapacity(
...
std::string SmallRoomCapacity::getSeats() const {
...
void SmallRoomCapacity::book(char row, int column) {
